...
localparam ROW   = 32'h44A00000;
localparam DTYPE = 32'h44A00004;
localparam COUNT = 32'h44A00008;
localparam DATA  = 32'h44A00020;
...
integer j;
  ...
  initial begin
    ...
    ***********************************************************************************************/
    ...
    // Write RGB
    writeRegister(DTYPE, 32'h0);
    for (i = 0; i < 12; i++)
    begin
        writeRegister(ROW, i);
        for (j = 0; j < 8; j++)
        begin
            writeRegister(DATA + j * 4, (i*8+j)<<20);
        end
    end

    // Write M_t
    writeRegister(DTYPE, 32'h1);
    for (i = 0; i < 4; i++)
    begin
        writeRegister(ROW, i);
        for (j = 0; j < 8; j++)
        begin
            writeRegister(DATA + j * 4, 32'h40404040);
        end
    end

    // Write V_t
    writeRegister(DTYPE, 32'h2);
    for (i = 0; i < 4; i++)
    begin
        writeRegister(ROW, i);
        for (j = 0; j < 8; j++)
        begin
            writeRegister(DATA + j * 4, 32'h10101010);
        end
    end
    ...
    // Read M_t
    writeRegister(DTYPE, 32'h5);
    for (i = 0; i < 4; i++)
    begin
        writeRegister(ROW, i);
        for (j = 0; j < 8; j++)
        begin
            readRegister(DATA + j * 4, Rdatabeat);
            $write("M_t %d value: 0x%08x\n", i*8+j, Rdatabeat[0]);
            agent.wait_drivers_idle();
        end
    end

    // Read V_t
    writeRegister(DTYPE, 32'h6);
    for (i = 0; i < 4; i++)
    begin
        writeRegister(ROW, i);
        for (j = 0; j < 8; j++)
        begin
            readRegister(DATA + j * 4, Rdatabeat);
            $write("V_t %d value: 0x%08x\n", i*8+j, Rdatabeat[0]);
            agent.wait_drivers_idle();
        end
    end

    // Read E_t
    writeRegister(DTYPE, 32'h7);
    for (j = 0; j < 4; j++)
    begin
        readRegister(DATA + j * 4, Rdatabeat);
        $write("E_t %d value: 0x%08x\n", j, Rdatabeat[0]);
        agent.wait_drivers_idle();
    end

    // Read count
    readRegister(COUNT, Rdatabeat);
