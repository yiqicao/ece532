...
void frame_intr_handler(void *CallbackRef, u32 Mask) {
	xil_printf("Interrupt handler called\n");
	if (XAxiVdma_IntrGetPending(&vdma, XAXIVDMA_WRITE) && XAXIVDMA_IXR_FRMCNT_MASK) {
...
void DemoInitialize()
{
	...
	//XAxiVdma_SetCallBack(&vdma, XAXIVDMA_IXR_FRMCNT_MASK, frame_intr_handler, (void *)&vdma, XAXIVDMA_WRITE);
...
void DemoRun()
{
	...
	while (userInput != 'q')
	{
		...
		while (XUartLite_IsReceiveEmpty(UART_BASEADDR) && !fRefresh && (userInput < 100))
		{
			usleep(10);
			userInput++;
		}
